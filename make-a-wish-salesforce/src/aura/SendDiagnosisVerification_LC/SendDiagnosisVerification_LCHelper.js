...
*/({
    ...
    handleMediProfessional: function(lead, component){
        ...
        if(!paperDVProcess){//Added as per IME-59
            _this.handleConfirmation(false, component, "Are you sure you want to submit Diagnosis Verification?", function(){
                ...
                } else if(checkMedProf) { 
                    _this.updateLeadRec('Treating Medical Professional', lead, component);
                } else {
                    _this.handleConfirmation(false, component, "Please provide Treating/Best physician/Alternate Medical Professional Email or Using Paper Process for DV.", function(){
                        _this.closeQuickAction();
                    }, true);
    ...
    updateLeadRec: function(medProfType, lead, component){
        ...
        if(lead.Part_A_Sent__c) { //has it sent before
            _this.handleConfirmation(false, component, "Are you sure you want to proceed? A Diagnosis Verification has already been sent and signed. Clicking Yes will reset all values in the DV form, and archive previous signed versions (in Docusign Status section) which cannot be undone.", function(){
                try {
