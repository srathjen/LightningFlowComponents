...
({
    save: function (component, event, helper) {
        ...
        var modifiedWishAffiliationJson = JSON.stringify(component.get("v.wishOther"));
        ...
        save.setCallback(this, function (response) {
            ...
            if (state === 'SUCCESS' && !Array.isArray(validationErrors) || !validationErrors.length) {
                // Set the Wish Other only if its not a Wish Change Request that was made
                if (isUserProfileAllowedToUpdateWishAffiliation) {
                    var wishOther = dataMap['wishAffiliation'];
                    component.set("v.wishOther",wishOther);
