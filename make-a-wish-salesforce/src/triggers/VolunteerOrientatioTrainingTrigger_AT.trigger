...
trigger VolunteerOrientatioTrainingTrigger_AT on Volunteer_Orientation_Training__c (Before insert, Before update, After insert, After update) {
	...
	if (trigger.isAfter && Trigger.isInsert) {
		...
		for (Volunteer_Orientation_Training__c newVolInsert : Trigger.new) {
			...
			if (newVolInsert.Type__c == 'Training' && newVolInsert.Volunteer_Attendance__c == 'Completed') {
		...
		//This method will update Affiliation as active and user profile as Active from prospective
		//when training record created status with completed.
		if (trainingCompletedVolunteerIdsSet.size() > 0) {
			VolunteerOandTTriggerHandler.UpdateAffiliationStatusAsActive(trainingCompletedVolunteerIdsSet);
		}
		...
		*/
		if (!trainingCompletedVolunteerIdsSet.isEmpty()) {
	...
	if (trigger.isAfter && Trigger.isUpdate) {
		...
		* Process Volunteers to try to Activate its Volunteer Role, User and Organization Affiliation
		*/
		if (!userUpdateContactId.isEmpty()) {
