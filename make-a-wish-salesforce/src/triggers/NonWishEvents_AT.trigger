 ...
 if(Trigger.isBefore)
 {
    if(Trigger.isInsert)
     ...
     if(chapterIds.size() > 0 && prioritysSet.size() > 0)
     {
           ...
           for(Non_Wish_Events__c currRec :[SELECT id, Chapter__c, Priority__c,event_date__c FROM Non_Wish_Events__c WHERE chapter__c IN :chapterIds AND Priority__c = :prioritysSet AND event_date__c  > : Date.Today()])
           {
     ...
     for(Non_Wish_Events__c  currRec : Trigger.new)
     {
       if(currRec.Priority__c != Null)
       {
         if(chapterPrioritySet.contains(currRec.Chapter__c+'-'+currRec.Priority__c))
         {
