...
currRec.addError('Active Background Check Already Exist'); 
    ...
    If(Trigger.isAfter && Trigger.isUpdate)
    {
        ...
        {
            if((dbBackgroundCheckRec.Status__c == 'Approved' || dbBackgroundCheckRec.Status__c ==  'Approved w/ Conditions') && dbBackgroundCheckRec.Status__c != Null && trigger.oldmap.get(dbBackgroundCheckRec.Id).Status__c != dbBackgroundCheckRec.Status__c && dbBackgroundCheckRec.Current__c == true)
            {
...
BackGroundCheckTriggerHandler.UpdateHiddenEmailField(volunteerIds,Trigger.new);
        ...
        if(approvedVolunteerIds.size() > 0)
        {
            ...
            BackGroundCheckTriggerHandler.updateVolunteerHiddenVolOTStatus(approvedVolunteerIds);
        }
        if(expirationDateMap.size() > 0)
        {
            BackGroundCheckTriggerHandler.UpdateVolunteerExpirationDate(expirationDateMap);
