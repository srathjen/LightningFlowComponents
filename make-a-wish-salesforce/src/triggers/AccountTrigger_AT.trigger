    ...
    if(trigger.isBefore && trigger.isInsert) {
        ...
        for(Account currentAccount : Trigger.new){
            if(Bypass_Triggers__c.getValues(userInfo.getUserId()) == Null) {
                if(currentAccount.RecordTypeId == inKindDonorsAccountRecordTypeId) {
                    ownerIdSet.add(currentAccount.OwnerId);
                    newInkAccountList.add(currentAccount);
                }
            }
        }
        if(newInkAccountList.size() > 0) {
            AccountTriggerHandler.matchChapterForInkindAccount(ownerIdSet, newInkAccountList);
        }
