trigger ContactFile_AT on cg__ContactFile__c (before insert, After insert) { 
    ...
    if(Trigger.isInsert && Trigger.isBefore) {
        ...
        if(conId.size() > 0) {
            for(cg__ContactFile__c contactFile : [SELECT Id, cg__Content_Type__c, cg__File_Name__c, cg__Contact__c, cg__Contact__r.Name, cg__Parent_Folder_Id__c FROM cg__ContactFile__c WHERE cg__Contact__c IN : conId AND cg__Content_Type__c = 'Folder' AND cg__File_Name__c = 'Documents']) {
